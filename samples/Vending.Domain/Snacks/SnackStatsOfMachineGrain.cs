...
/// <summary>
///     Grain implementation class SnackStatsOfMachineGrain.
/// </summary>
[StorageProvider(ProviderName = Constants.GrainStorageName)]
public class SnackStatsOfMachineGrain : StatefulGrainWithGuidKey<MachineStat, SnackEvent, SnackErrorEvent>, ISnackStatsOfMachineGrain
{
    private readonly DomainDbContext _dbContext;
    private readonly ILogger<SnackStatsOfMachineGrain> _logger;

    /// <inheritdoc />
    public SnackStatsOfMachineGrain(DomainDbContext dbContext, ILogger<SnackStatsOfMachineGrain> logger)
    ...
    /// <inheritdoc />
    public Task<MachineStat> GetStateAsync()
    {
    ...
    public Task<Result> IncrementCountAsync(SnackIncrementMachineCountCommand command)
    {
        ...
        return ValidateIncrementCount(command)
              ...
              .MapTryAsync(() => PublishAsync(new MachineCountUpdatedEvent(snackId, State.Count, command.TraceId, command.OperatedAt, command.OperatedBy)))
              .TapErrorTryAsync(errors => PublishErrorAsync(new SnackErrorEvent(snackId, 0, 131, errors.ToReasonStrings(), command.TraceId, DateTimeOffset.UtcNow, command.OperatedBy)));
    }
    ...
    public Task<Result> DecrementCountAsync(SnackDecrementMachineCountCommand command)
    {
        ...
        return ValidateDecrementCount(command)
              ...
              .MapTryAsync(() => PublishAsync(new MachineCountUpdatedEvent(snackId, State.Count, command.TraceId, command.OperatedAt, command.OperatedBy)))
              .TapErrorTryAsync(errors => PublishErrorAsync(new SnackErrorEvent(snackId, 0, 132, errors.ToReasonStrings(), command.TraceId, DateTimeOffset.UtcNow, command.OperatedBy)));
    }
    ...
    private async Task UpdateCountAsync()
    {
        ...
        try
        {
            var count = await _dbContext.Machines.Where(sm => sm.IsDeleted == false && sm.Slots.Any(sl => sl.SnackPile != null && sl.SnackPile.SnackId == snackId)).CountAsync();
