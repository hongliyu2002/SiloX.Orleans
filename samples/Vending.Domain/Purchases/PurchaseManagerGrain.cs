...
public sealed class PurchaseManagerGrain : Grain, IPurchaseManagerGrain
{
    ...
    public Task<Result<ImmutableList<Purchase>>> ListAsync(PurchaseManagerListQuery query)
    {
        ...
        return Result.Ok(_dbContext.Purchases.AsNoTracking())
                     .MapIf(machineId != null, purchases => purchases.Where(s => s.MachineId == machineId))
                     .MapIf(snackId != null, purchases => purchases.Where(s => s.SnackId == snackId))
                     .MapIf(boughtPriceRangeStart != null, purchases => purchases.Where(s => s.BoughtPrice >= boughtPriceRangeStart))
                     .MapIf(boughtPriceRangeEnd != null, purchases => purchases.Where(s => s.BoughtPrice < boughtPriceRangeEnd))
                     .MapIf(boughtAtRangeStart != null, purchases => purchases.Where(s => s.BoughtAt >= boughtAtRangeStart))
                     .MapIf(boughtAtRangeEnd != null, purchases => purchases.Where(s => s.BoughtAt < boughtAtRangeEnd))
                     .MapIf(boughtBy != null, purchases => purchases.Where(s => s.BoughtBy == boughtBy))
