...
/// <summary>
///     Extensions for <see cref="IClusterClient" />.
/// </summary>
public static class ClusterClientExtensions
{
    ...
    public static IObservable<(TEvent Event, StreamSequenceToken SequenceToken)> GetSubscriberStreamWithGuidKey<TEvent>(this IClusterClient clusterClient, string providerName, string subNamespace, Guid id, StreamSequenceToken? sequenceToken = null)
    ...
    /// <returns>An <see cref="IObservable{T}" /> that will emit events as they are received.</returns>
    public static IObservable<(TEvent Event, StreamSequenceToken SequenceToken)> GetReceiverStreamWithGuidKey<TEvent>(this IClusterClient clusterClient, string providerName, string broadcastNamespace, StreamSequenceToken? sequenceToken = null)
        where TEvent : DomainEvent
    {
        Guard.Against.Null(clusterClient, nameof(clusterClient));
        Guard.Against.NullOrWhiteSpace(providerName, nameof(providerName));
    ...
    /// <param name="sequenceToken">The sequence token to start from. If you want to start from the beginning of the stream, you can pass null.</param>
    /// <returns>An <see cref="IObservable{T}" /> that will emit events as they are received.</returns>
    public static IObservable<(TEvent Event, StreamSequenceToken SequenceToken)> GetSubscriberStreamWithStringKey<TEvent>(this IClusterClient clusterClient, string providerName, string subNamespace, string id, StreamSequenceToken? sequenceToken = null)
        where TEvent : DomainEvent
    {
        Guard.Against.Null(clusterClient, nameof(clusterClient));
        Guard.Against.NullOrWhiteSpace(providerName, nameof(providerName));
    ...
    /// <typeparam name="TEvent">The type of event to receive. This must be a type that is serializable by Orleans.</typeparam>
    /// <returns>An <see cref="IObservable{T}" /> that will emit events as they are received.</returns>
    public static IObservable<(TEvent Event, StreamSequenceToken SequenceToken)> GetReceiverStreamWithStringKey<TEvent>(this IClusterClient clusterClient, string providerName, string broadcastNamespace, StreamSequenceToken? sequenceToken = null)
        where TEvent : DomainEvent
    {
        Guard.Against.Null(clusterClient, nameof(clusterClient));
        Guard.Against.NullOrWhiteSpace(providerName, nameof(providerName));
